...
export const MEDIA_ACTIONS: MediaActionDefinition[] = [
  ...
  },
  {
    ...
    nodeKind: 'Transform',
    label: 'Trim Video',
    description: 'Cut a video to a selected local start and duration.',
    mediaType: 'video',
    workflowType: 'video-utility',
    inputs: [input('video', 'video')],
    outputs: [output('video', 'video')],
    controls: [
      { id: 'startMs', label: 'Start ms', type: 'number', defaultValue: 0, min: 0, step: 1 },
      { id: 'durationMs', label: 'Duration ms', type: 'number', defaultValue: 5000, min: 1, step: 1 },
    ],
    providerPreference: ['local'],
    executor: 'ffmpeg',
    costEstimate: 0,
  ...
  {
    actionId: 'video.concat',
    nodeKind: 'Combine',
    label: 'Concatenate Videos',
    description: 'Join local video clips into one output.',
    mediaType: 'video',
    workflowType: 'video-utility',
  ...
  {
    actionId: 'video.merge-audio-video',
    nodeKind: 'Combine',
    label: 'Merge Audio and Video',
    description: 'Combine a local video track with a local audio track.',
    mediaType: 'video',
    workflowType: 'video-utility',
    inputs: [input('video', 'video'), input('audio', 'audio', { position: 'top' })],
    outputs: [output('video', 'video')],
    providerPreference: ['local'],
    executor: 'ffmpeg',
    costEstimate: 0,
  ...
  {
    actionId: 'video.extract-frame',
    nodeKind: 'Transform',
    label: 'Extract Frame',
    description: 'Extract a single local video frame at a timestamp.',
    mediaType: 'image',
    workflowType: 'video-utility',
    inputs: [input('video', 'video')],
    outputs: [output('image', 'image')],
    controls: [{ id: 'atMs', label: 'At ms', type: 'number', defaultValue: 0, min: 0, step: 1 }],
    providerPreference: ['local'],
    executor: 'ffmpeg',
    costEstimate: 0,
    batchPolicy: 'map',
  ...
  {
    actionId: 'video.scale',
    nodeKind: 'Transform',
    label: 'Scale Video',
    description: 'Resize a local video to a target width and height.',
    mediaType: 'video',
    workflowType: 'video-utility',
    inputs: [input('video', 'video')],
    outputs: [output('video', 'video')],
    controls: [
      { id: 'width', label: 'Width', type: 'number', defaultValue: 1920, min: 1, step: 1 },
      { id: 'height', label: 'Height', type: 'number', defaultValue: 1080, min: 1, step: 1 },
      toggle('preserveAspectRatio', 'Preserve Aspect', true),
    ],
    providerPreference: ['local'],
    executor: 'ffmpeg',
    costEstimate: 0,
  ...
  {
    actionId: 'video.compose',
    nodeKind: 'Combine',
    label: 'Compose Timeline',
    description: 'Compose local video, image, and audio inputs into a timeline.',
    mediaType: 'video',
    workflowType: 'video-utility',
    inputs: [
      input('visual', 'any', { cardinality: 'n' }),
      input('audio', 'audio', { cardinality: 'n', optional: true, position: 'top' }),
    ],
    outputs: [output('video', 'video')],
    providerPreference: ['local'],
    executor: 'ffmpeg',
  ...
  {
    actionId: 'video.blend',
    nodeKind: 'Combine',
    label: 'Blend Videos',
    description: 'Blend two local video sources together.',
    mediaType: 'video',
    workflowType: 'video-utility',
    inputs: [input('videoA', 'video'), input('videoB', 'video', { position: 'top' })],
    outputs: [output('video', 'video')],
    controls: [select('mode', 'Mode', 'overlay', [
  ...
  {
    actionId: 'video.interleave',
    nodeKind: 'Combine',
    label: 'Interleave Video',
    description: 'Interleave frames from local video sources.',
    mediaType: 'video',
    workflowType: 'video-utility',
    inputs: [input('video', 'video', { cardinality: 'n' })],
    outputs: [output('video', 'video')],
    providerPreference: ['local'],
    executor: 'ffmpeg',
    costEstimate: 0,
  ...
  {
    actionId: 'video.metadata',
    nodeKind: 'Transform',
    label: 'Probe Video Metadata',
    description: 'Read duration, codec, frame rate, and dimensions from local media.',
    mediaType: 'json',
    workflowType: 'video-utility',
    inputs: [input('video', 'video')],
    outputs: [output('metadata', 'json')],
    providerPreference: ['local'],
    executor: 'ffmpeg',
    costEstimate: 0,
    batchPolicy: 'map',
  ...
  {
    actionId: 'audio.merge',
    nodeKind: 'Combine',
    label: 'Merge Audio Files',
    description: 'Mix or concatenate local audio files.',
    mediaType: 'audio',
    workflowType: 'audio-utility',
    inputs: [input('audio', 'audio', { cardinality: 'n' })],
    outputs: [output('audio', 'audio')],
    controls: [select('mode', 'Mode', 'mix', [
      { label: 'Mix', value: 'mix' },
  ...
  {
    actionId: 'audio.loudness-normalize',
    nodeKind: 'Transform',
    label: 'Loudness Normalize',
    description: 'Normalize local audio loudness with FFmpeg loudnorm.',
    mediaType: 'audio',
    workflowType: 'audio-utility',
    inputs: [input('audio', 'audio')],
    outputs: [output('audio', 'audio')],
    controls: [
      slider('integratedLufs', 'Integrated LUFS', -16, -30, -6, 0.5),
      slider('truePeak', 'True Peak', -1.5, -6, 0, 0.1),
    ],
    providerPreference: ['local'],
    executor: 'ffmpeg',
    costEstimate: 0,
  ...
  {
    actionId: 'audio.waveform',
    nodeKind: 'Transform',
    label: 'Extract Waveform',
    description: 'Extract local waveform peak data for scrubbing and previews.',
    mediaType: 'json',
    workflowType: 'audio-utility',
    inputs: [input('audio', 'audio')],
    outputs: [output('waveform', 'json')],
    controls: [{ id: 'resolution', label: 'Resolution', type: 'number', defaultValue: 1024, min: 64, max: 16384, step: 64 }],
    providerPreference: ['local'],
    executor: 'ffmpeg',
    costEstimate: 0,
    batchPolicy: 'map',
  ...
  {
    actionId: 'audio.compressor',
    nodeKind: 'Transform',
    label: 'Audio Compressor',
    description: 'Apply local dynamic range compression.',
    mediaType: 'audio',
    workflowType: 'audio-utility',
    inputs: [input('audio', 'audio')],
    outputs: [output('audio', 'audio')],
    controls: [
      slider('threshold', 'Threshold', -18, -60, 0, 1),
      slider('ratio', 'Ratio', 4, 1, 20, 0.5),
      slider('attackMs', 'Attack ms', 20, 0, 200, 1),
      slider('releaseMs', 'Release ms', 250, 10, 1000, 10),
    ],
    providerPreference: ['local'],
    executor: 'ffmpeg',
    costEstimate: 0,
  ...
  },
  {
    actionId: 'fal.ffmpeg',
    nodeKind: 'Transform',
    label: 'Legacy Fal FFmpeg',
    description: 'Legacy cloud-only Fal-hosted FFmpeg fallback. Default desktop utility nodes use local FFmpeg actions instead.',
    mediaType: 'json',
