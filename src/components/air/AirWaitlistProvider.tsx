...
export default function AirWaitlistProvider({ children }: { children: ReactNode }) {
  ...
  const close = useCallback(() => { dialogRef.current?.close(); setOpen(false); window.setTimeout(() => restoreFocusRef.current?.focus({ preventScroll: true }), 0); }, []);
