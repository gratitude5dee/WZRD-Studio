...
import { timeToX, xToTimeMs } from '@/lib/editor/timelineZoom';
...
const TimelinePlayhead: React.FC<TimelinePlayheadProps> = ({
  ...
  const position = Math.max(0, timeToX(currentTimeMs, pixelsPerSecond) - scrollOffset);
  const isDraggingRef = useRef(false);
  const playheadRef = useRef<HTMLDivElement>(null);
  const label = formatTimecode(currentTimeMs);
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      const handleMouseMove = (moveEvent: MouseEvent) => {
        ...
        const timeMs = Math.max(0, Math.min(durationMs, xToTimeMs(x, pixelsPerSecond)));
        onSeekMs(timeMs);
      };
    ...
    [onSeekMs, durationMs, pixelsPerSecond, scrollOffset]
  );
