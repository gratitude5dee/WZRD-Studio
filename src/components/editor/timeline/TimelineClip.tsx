...
import { useVideoEditorStore, Clip, AudioTrack, type Keyframe } from '@/store/videoEditorStore';
import { moveSelection } from '@/features/editor-opencut/openCutCommands';
import {
  ContextMenu,
  ContextMenuContent,
  ContextMenuItem,
...
const formatKeyframeTime = (timeMs: number) => {
...
const getKeyframeLeftPx = (keyframe: Keyframe, clip: Clip | AudioTrack, zoom: number) =>
  (((keyframe.time ?? 0) - (clip.startTime ?? 0)) / 1000) * zoom;

export function TimelineClip({ clip, zoom, onSelect, isSelected, isTrackLocked = false }: TimelineClipProps) {
  ...
  const clipEnd = clip.endTime ?? clipStart + duration;
  const targetKeyframes = useMemo(
    () =>
      keyframes
        .filter((keyframe) => keyframe.targetId === clip.id && keyframe.time >= clipStart && keyframe.time <= clipEnd)
        .sort((a, b) => a.time - b.time),
    [clip.id, clipEnd, clipStart, keyframes]
  );
  ...
  const [{ isDragging }, dragRef] = useDrag({
    ...
    item: { id: clip.id, startTime: clip.startTime },
      ...
      const isSelected = clip.type === 'audio'
        ? state.selectedAudioTrackIds.includes(clip.id)
        : state.selectedClipIds.includes(clip.id);
      if (!isSelected) {
        if (clip.type === 'audio') {
          selectAudioTrack(clip.id);
        } else {
          selectClip(clip.id);
        }
      }
      moveSelection({ deltaMs: targetStart - (clip.startTime ?? 0) });
    },
    collect: (monitor) => ({
  ...
  const handleKeyframeSelect = (keyframe: Keyframe) => (event: MouseEvent<HTMLButtonElement>) => {
    event.stopPropagation();
      ...
      <ContextMenuTrigger asChild>
        <div
          ...
          style={{
            ...
            opacity: isTrackLocked ? 0.45 : isDragging || isTrimming ? 0.5 : 1,
            cursor: isTrackLocked ? 'not-allowed' : 'move',
          ...
          {targetKeyframes.map((keyframe) => {
            ...
            return (
              <button
                key={keyframe.id}
                type="button"
                aria-label={`Select keyframe ${propertyLabel} for ${clip.name || 'Unnamed'} at ${formatKeyframeTime(keyframe.time)}`}
                aria-pressed={selected}
                className="absolute z-20 h-3 w-3 border transition-colors focus:outline-none focus:ring-2 focus:ring-white/80"
                data-testid={`editor-timeline-keyframe-${keyframe.id}`}
                data-timeline-interactive="true"
                onClick={handleKeyframeSelect(keyframe)}
                style={{
                  left: `${getKeyframeLeftPx(keyframe, clip, zoom)}px`,
                  top: '50%',
                ...
                title={`${propertyLabel} ${formatKeyframeTime(keyframe.time)}`}
              />
            );
          })}
