...
const ShotForm: React.FC<ShotFormProps> = ({
  ...
  const fallbackGeneratingRef = React.useRef(false);
  const noopBooleanSetter = React.useCallback(() => {}, []);
  const noopStringSetter = React.useCallback(() => {}, []);
  const noopNullableStringSetter = React.useCallback(() => {}, []);
  const noopImageStatusSetter = React.useCallback(() => {}, []);
  const noopAudioStatusSetter = React.useCallback(() => {}, []);

  const canGenerateVisual = Boolean(
    id &&
    setIsGeneratingPrompt &&
    setIsGeneratingImage &&
    setLocalVisualPrompt &&
  ...
  const aiGeneration = useAIGeneration({
    shotId: id ?? '',
    isGeneratingRef: isGeneratingRef ?? fallbackGeneratingRef,
    setIsGeneratingPrompt: setIsGeneratingPrompt ?? noopBooleanSetter,
    setIsGeneratingImage: setIsGeneratingImage ?? noopBooleanSetter,
    setLocalVisualPrompt: setLocalVisualPrompt ?? noopStringSetter,
    setLocalImageStatus: setLocalImageStatus ?? noopImageStatusSetter,
  ...
  const audioGeneration = useAudioGeneration({
    shotId: id ?? '',
    isGeneratingRef: isGeneratingRef ?? fallbackGeneratingRef,
    setIsGeneratingAudio: setIsGeneratingAudio ?? noopBooleanSetter,
    setLocalAudioUrl: setLocalAudioUrl ?? noopNullableStringSetter,
    setLocalAudioStatus: setLocalAudioStatus ?? noopAudioStatusSetter
  });

  const handleGenerateVisualPrompt = canGenerateVisual ? aiGeneration.handleGenerateVisualPrompt : () => {};
  const handleGenerateImage = canGenerateVisual ? aiGeneration.handleGenerateImage : () => {};
  const handleGenerateAudio = canGenerateAudio ? audioGeneration.handleGenerateAudio : async () => {};
        ...
        <div className="mt-2">
          ...
          {visualPrompt && ['prompt_ready', 'failed', 'completed'].includes(imageStatus) && (
            <Button
              variant="outline"
              size="sm"
              className="mt-1 h-7 w-full border-[#f97316]/20 bg-black/30 text-xs text-[#fdba74] backdrop-blur-sm hover:bg-[#1a1510] hover:text-[#FDE8D0]"
        ...
        <div className="flex justify-end space-x-2 pt-1 mt-2">
          ...
          {onSave && (
            <Button
              ...
              className="h-7 bg-[#221a10] text-xs text-[#FDE8D0] backdrop-blur-sm hover:bg-[#2d2210]"
            >
              Save
            </Button>
          )}
