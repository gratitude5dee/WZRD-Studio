...
import { getModelCredits } from '@/lib/constants/credits';
import { getDefaultVideoModel } from '@/lib/studio-model-constants';
...
const ImagesToVideoNode = memo(({ data }: ImagesToVideoNodeProps) => {
  ...
  const { useCredits: spendCredits, availableCredits } = useCredits();
  const { user } = useAuth();
  const navigate = useNavigate();

  const videoCreditCost = useMemo(() => {
    const defaultModelId = getDefaultVideoModel();
    return getModelCredits(defaultModelId) ?? 20;
  }, []);
  ...
  const handleGenerate = async () => {
    ...
    if (availableCredits === 0) {
      toast.error('You need credits to generate videos. Redirecting to billing.');
      navigate('/settings/billing');
      return;
    }
    
    const creditUsed = await spendCredits('video', videoCreditCost, {
      ...
      <div className="flex items-center justify-between p-4 bg-zinc-900/30">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 text-zinc-400 text-xs">
            ...
            <span>{videoCreditCost} credits</span>
          </div>
        </div>
        <button
          onClick={handleGenerate}
          disabled={images.filter(img => img !== null).length < 2 || !user || availableCredits < videoCreditCost}
