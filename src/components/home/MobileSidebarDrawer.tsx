import { X, FolderKanban, Layers, Users, Globe, Settings, HelpCircle, LogOut, Sparkles, Images, ShieldCheck, Scissors, DatabaseZap, CalendarDays, Clapperboard, Music2, ChevronDown } from 'lucide-react';
...
import { openExternalNavItem, WTR_EXTERNAL_URL } from './Sidebar';
...
export const MobileSidebarDrawer = ({ isOpen, onClose, activeView, onViewChange }: MobileSidebarDrawerProps) => {
  ...
  ] as Array<{ id: string; label: string; description?: string; icon: typeof FolderKanban; isRoute?: boolean; path?: string; externalUrl?: string; showBadge?: boolean }>;

  const clipStudioNavItems = [
      ...
      <aside className={cn(
        ...
        <nav className="flex-1 p-4 space-y-6 overflow-y-auto">
          ...
          <div>
              ...
              {mainNavItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {item.showBadge && (
                      <Badge variant="secondary" className="shrink-0 text-[9px] bg-primary/20 text-primary border-primary/30">
                        New
                      </Badge>
                    )}
                  </button>
            ...
            <div className="mt-1">
              <button
                onClick={() => setClipStudioOpen((open) => !open)}
                aria-label="Clip Studio"
                aria-expanded={clipStudioOpen}
                className="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-all"
              >
                <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-muted/50 shrink-0">
                  <Clapperboard className="w-4 h-4" />
                </div>
                <span className="flex-1 min-w-0 truncate text-left">Clip Studio</span>
                <ChevronDown className={cn("w-4 h-4 shrink-0 transition-transform duration-200", clipStudioOpen ? "rotate-0" : "-rotate-90")} />
              </button>
              {clipStudioOpen && (
                <div className="mt-1 ml-6 pl-3 border-l border-border/50 space-y-1">
                  {clipStudioNavItems.map((item) => {
                    const Icon = item.icon;
                    const isActive = activeView === item.id;
                    return (
                      <button
          ...
          <div>
            <div className="flex items-center gap-2 px-3 mb-3">
              ...
              <span className="text-[10px] font-semibold text-muted-foreground/70 uppercase tracking-[0.15em]">Studio</span>
            </div>
            <div className="space-y-1">
