...
const StoryboardPage = () => {
  ...
  const shotsGenerationTimeoutRef = useRef<ReturnType<typeof setTimeout> | undefined>(undefined);
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0A0A0F] relative overflow-hidden">
      ...
      <div className="fixed top-[-20%] left-[-10%] w-[60%] h-[60%] bg-[radial-gradient(ellipse,rgba(255,107,74,0.12)_0%,transparent_70%)] pointer-events-none z-0" />
      <div className="fixed bottom-[-20%] right-[-10%] w-[50%] h-[50%] bg-[radial-gradient(ellipse,rgba(234,88,12,0.08)_0%,transparent_70%)] pointer-events-none z-0" />
      <AppHeader />
          ...
          <div className="p-6 h-full overflow-y-auto relative">
            {scenes.length === 0 ? (
              ...
              >
                ...
                <Button 
                  variant="outline" 
                  className="border-orange-500/30 text-orange-300 hover:bg-orange-950/30"
                  onClick={addScene}
                >
              ...
              <AnimatePresence initial={false}>
                {scenes.map((scene, index) => (
                  <motion.div 
                    ...
                    className={`${selectedScene?.id === scene.id ? 'border-l-2 border-orange-500 pl-4 -ml-4 mb-12' : 'mb-12'}`}
                  >
                    <ShotsRow
                      sceneId={scene.id}
            ...
            <motion.button
              onClick={addScene}
              className="fixed bottom-6 right-6 rounded-full h-14 w-14 bg-gradient-to-br from-orange-500 to-red-500 p-0 flex items-center justify-center shadow-lg hover:scale-105 transition-transform z-20"
              whileHover={{ 
                scale: 1.1, 
                rotate: 15,
                boxShadow: "0px 0px 20px 0px rgba(255, 107, 74, 0.5)"
              }}
              whileTap={{ scale: 0.95 }}
              transition={{ 
                type: "spring", 
